...
const Topbar = ({ selectedMenuItem, toggleSidebar, isSidebarOpen }) => {
  ...
  const handleClickOutside = (event) => {
    if (!event.target.closest('.sidebar') && !event.target.closest('.bars-icon')) {
      toggleSidebar(false); // Close sidebar if clicked outside
    }
  };
  ...
  return (
    <div className="top-navbar d-flex justify-content-between">
      <button className="toggle-btn bars-icon" style={{ border: 'none' }} onClick={() => toggleSidebar(true)}>
        <FontAwesomeIcon icon={faBars} />
      </button>
      <div className='page-name'>
        {selectedMenuItem}
