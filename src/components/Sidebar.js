...
//     ) {
//       setDropdownState({ administration: true });
//     }
//   }, [location]);
...
import { Link, useLocation } from "react-router-dom";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faUser, faSitemap, faUserPlus, faGear, faUserPen, faXmark } from "@fortawesome/free-solid-svg-icons";
import { useTranslation } from "react-i18next";
import {
...
const Sidebar = ({setSelectedMenuItem, toggleSidebar, userPermissions}) => {
  ...
  // Load active menu from localStorage if available, else fallback to current pathname
  const [activeMenu, setActiveMenu] = useState(localStorage.getItem("activeMenu") || location.pathname);
  ...
  // Automatically open the dropdown if a nested item is active
  useEffect(() => {
    const currentPath = location.pathname;
    if (
      currentPath.includes("tenants") ||
        ...
        <aside className="sidebar">
          ...
          <nav className="menu">
            <ul className="">
              ...
              >
                <li className={`sidenav-menuitem ${activeMenu === "Data Management" ? "active-menu" : ""}`}>
                  <img src={Datamanagementicon} />
                  {t("Data Management")}
                </li>
              </Link>
